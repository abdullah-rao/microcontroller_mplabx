...
#pragma config   FPLLIDIV  = DIV_12  // PLL in 8mHz/10 = 800 kHz
...
int global_delay_val = 49503;//3sec 1 sec is 16501       // vary this to change the pulse's delay.
...
int main(int argc, char** argv) {
	...
	LATBbits.LATB2= 0;				// Set value of PORT A output to 0. 
	TRISBbits.TRISB2 = 0;				// Set all pins on PORT A to output
    ...
    while(1) 
    {
    	...
    	LATBbits.LATB2 = 0x0001;		// Set Pin 0 on PORT A to logic level "high" (3v).    

        // Turn LED off (takes roughly 1 sec)
        delay(global_delay_val);         // 1000: roughly Delay for roughly 1 second with 187.5 kHz system clock
		LATBbits.LATB2 = 0x0000;		// Set Pin 0 on PORT A to logic level "low" (0v).
